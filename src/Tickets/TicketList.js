...
const iconMapping = {
  0: <AiOutlineEllipsis />,
  1: <FcHighPriority />,
  2: <MdSignalCellularAlt />,
  3: <MdSignalCellularAlt2Bar />,
  4: <MdSignalCellularAlt1Bar />,
};

const priorityNameMapping = {
  0: "No priority",
  1: "Urgent",
  2: "High",
  3: "Medium",
    ...
    <div className="main">
      <div className={`dropdown-button ${isDropdownOpen ? "open" : ""}`}>
        <button onClick={() => setDropdownOpen(!isDropdownOpen)}>
          <span className="hamburger-icon">&#9776;</span> Display{" "}
            ...
            <h2>
              ...
              users.length > 0 ? (
                <div className="users-list-count">
                  {groupKey}: {groupedAndSorted[groupKey].length}
                </div>
              ) : groupingOption === "priority" ? (
                <div className="icon-priority">
                  {iconMapping[groupKey]} {priorityNameMapping[groupKey]}:{" "}
                  {groupedAndSorted[groupKey].length}
                </div>
              ) : (
                <div className="users-list-count">
                  {groupKey}: {groupedAndSorted[groupKey].length}
              ...
              {groupedAndSorted[groupKey].map((ticket) => (
                <div key={ticket.id} className="ticket-card">
                  ...
                  <div className="feature_priority">
                    {groupingOption === "user" && (
                      <div>{iconMapping[ticket.priority]}</div>
                    )}
